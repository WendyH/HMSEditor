...
namespace FastColoredTextBoxNS {
	public sealed class SyntaxHighlighter: IDisposable {
		...
		private Regex CSharpAttributeRegex, CSharpStringAndCommentsRegex, // By WendyH
		...
		public void HighlightSyntax(Language language, Range range) {
			switch (language) {
				...
				case Language.CPPScript:
					CPPScriptSyntaxHighlight(range);
					break;
				case Language.PascalScript:
					PascalScriptSyntaxHighlight(range);
					break;
				case Language.BasicScript:
					BasicSyntaxHighlight(range);
					break;
				case Language.JScript:
					HmsJScriptSyntaxHighlight(range);
					break;
				case Language.YAML:
...
//			VBStringRegex    = new Regex(@"""(\\[\s\S]|[^""])*""|('.*)", RegexCompiledOption);  // By WendyH
		...
		void InitCPPScriptRegex() {
			...
			string keywords = "#include|#define|new|break|continue|exit|delete|return|if|else|switch|default|case|do|while|for|try|finally|except|in|is|int|long|void|bool|float|" + hmsCommonTypes;
			CPPScriptKeywordRegex = new Regex(@"(?:^|\b)(" + keywords   + @")\b", RegexCompiledOption | RegexOptions.IgnoreCase | RegexOptions.Multiline);
