...
namespace HMSEditorNS {
	...
	public class EditorList: List<HMSEditor> {
		public bool ContainsScriptFrame(IntPtr hWnd) {
			foreach (HMSEditor o in this) if (o.IsScriptFrameHWnd(hWnd)) return true;
			return false;
		}

		public HMSEditor FindByHandle(IntPtr hWnd) {
			foreach (HMSEditor o in this)
				if (o.KnownHMSControlHWnd(hWnd)) return o;
			return null;
		}

		public HMSEditor FindByHandleButtonRunOrStep(IntPtr hWnd) {
			foreach (HMSEditor o in this)
				if (o.IsHWndButtonRunOrStep(hWnd)) return o;
			return null;
		}

		public HMSEditor this[IntPtr hWnd] {
			get {
				foreach (HMSEditor o in this) if (o.IsScriptFrameHWnd(hWnd)) return o;
				HMSEditor item = new HMSEditor(hWnd);
				Add(item);
				return item;
			}
		}

		public void New(IntPtr hWnd) {
			foreach (HMSEditor o in this) if (o.IsScriptFrameHWnd(hWnd)) return;
			HMSEditor item = new HMSEditor(hWnd);
			Add(item);
		}
	}
